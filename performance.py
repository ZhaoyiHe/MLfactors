...
rf_factor_data = rf_factor_data.rename(columns={"Unnamed: 0":"date"}).set_index('date').stack().reset_index().rename(columns={"level_1":"order_book_id",0:"factor_value"})
...
class Evaluation(object):
    ...
    def _match_data(self):
        ...
        self.factor_vs_return = pd.merge(self.factor_data, self.next_stock_return, how="inner",
                                         on=["order_book_id", "date"]).dropna(axis=0).set_index(["date","order_book_id"])
    ...
    def calculate_IC(self):
        ...
        IC_df = pd.DataFrame(
            data = self.factor_vs_return.groupby(level="date").apply(lambda x:  stats.spearmanr(x.iloc[:,-1], x.iloc[:,-2])[0])
    ...
    def OLS_reg(self):
        ...
        factor_return_df = pd.DataFrame(
            data = self.factor_vs_return.groupby(level="date").apply(lambda x:  sm.OLS(x.iloc[:,-1], x.iloc[:,-2]).fit().params[0])
            , index= self.factor_vs_return.index.get_level_values("date").unique())
        self.factor_return =(1+factor_return_df).cumprod().iloc[-1]
